...
int main(int argc, char *argv[])
{
	  ...
	  for (j=0; j < L; j++)
	    {
	      ...
	      if(r < rho)
		{
		  map[i][j] = 0; // zero indicates rock
		}
	      else
		{
		  nhole++;
		  map[i][j] = nhole; // a positive value indicates a hole 1,2,3,4....
		}
	    }
	}
  ...
  /*
   *  Now scatter the map to smallmap
   */
  ...
  while (step <= maxstep)
      ...
      for (i=1; i<=M; i++)
	{
	  for (j=1; j<=N; j++)
         ...
         * set new[i][j]=max(old[i][j],old[i-1][j],old[i+1][j],old[i][j-1],old[i,j+1])
	       */

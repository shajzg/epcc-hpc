 ...
 *  shown as shades of grey on a black background, with the largest
 *  cluster shown in white.
 */

#define MAXNCLUSTER 9          // Must be able to identify by a single digit
int clusterfound[MAXNCLUSTER];
...
void percwrite(char *mapfile, int map[L][L], int ncluster)
{
  ...
  /*
   * Find the size of the "ncluster" largest clusters (using brute force!)
   */
  ...
  for (icluster = 0; icluster < ncluster; icluster++)
      ...
      for (i=0; i<=MAXCLUSTERID; i++)
	{
      ...
      clusterfound[icluster] = maxcluster;
  ...
  for (icluster = 0; icluster < ncluster-1; icluster++)
    {
      printf("%d, ", clusterfound[icluster]);
    }
  printf("%d\n", clusterfound[ncluster-1]);
	  ...
	  if (clusterid > 0)
	    {
	      for (icluster = 0; icluster < ncluster; icluster++)
		{
		  if (clustersize[clusterid] == clusterfound[icluster])
		    {
 ...
 *  Function to write a percolation map in greyscale Portable Grey Map
 *  (PGM) format. The largest "ncluster" clusters are identified and
 *  shown as shades of grey on a black background, with the largest cluster
 *  shown in white.
 *

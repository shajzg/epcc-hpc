...
int main(int argc, char *argv[])
{
  ...
  int jleft, jright, ipbcstart, ipbcstop, perc;
  double r;
  ...
  while (step <= maxstep)
    {
      /*
       *  Implement periodic boundaries in the second dimension "j",
       *  i.e. map j=0 to j=L and j=L+1 to j=1
       */
       ...
       /*
        *  Map is only periodic in the central three-fifths of the domain
        *  so reset the left and right extremes to zero. It may seem
        *  wasteful to copy the halos then zero some of them, but this
        *  approach is probably the simplest to implement in parallel.
        */
       ...
       for (i=1; i <= L; i++)
         {
           if (i < ipbcstart || i > ipbcstop)
             {
               // Periodic boundaries do not operate in this regime
